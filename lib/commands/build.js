    ...
    run(options, config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (hasCliArgumentsPassed) {
                yield build(this.commandOptionsToBuildOptions(options), polymerProject, config);
                return;
            }
            ...
            if (config.builds) {
                const promises = config.builds.map((buildOptions) => build(buildOptions, polymerProject, config));
            ...
            yield build({}, polymerProject, config);
        });
    }
}
